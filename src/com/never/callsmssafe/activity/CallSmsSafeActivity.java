...
import com.never.callsmssafe.utils.UIUtils;
				...
				public void onClick(View v) {
					new Thread() {
						public void run() {
							...
							if (result) {
								UIUtils.showToast(CallSmsSafeActivity.this,
										"删除成功");
								// 更新界面
								// 从ui显示的集合infos中删除条目
								infos.remove(info);
		...
		dialogView.findViewById(R.id.bt_ok).setOnClickListener(
				new OnClickListener() {
					...
					public void onClick(View v) {
						...
						//判断号码是否存在
						boolean flag=dao.isBlockMode(number);
						if(flag==true){
							Toast.makeText(getApplicationContext(), "号码已经存在！！", 1)
							.show();
							return;
						}
